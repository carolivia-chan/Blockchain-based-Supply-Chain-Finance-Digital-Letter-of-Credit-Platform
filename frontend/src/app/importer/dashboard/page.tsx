...
export default function ImporterDashboard() {
  ...
  // Status display - REMOVED InTransit
    ...
    try {
      ...
      if (confirmProductId) {
        ...
        setProductInfo({
          ...
          canConfirm: Number(product.status) === 1 // Delivered (index 1 after removing InTransit)
        });
      }
      ...
      if (confirmLcId) {
        ...
        setLcInfo({
          ...
          statusName: LC_STATUS[Number(lcData.status)],
          canConfirm: Number(lcData.status) === 3 // DELIVERED_PENDING
        });
  ...
  async function confirmProductReceived() {
    ...
    try {
      ...
      if (err.message?.includes("Only Importer")) {
        ...
        errorMsg = "❌ Product must be Delivered first";
      } else if (err.reason) {
        errorMsg = `❌ ${err.reason}`;
      } else if (err.message) {
              ...
              <div style={{
                ...
                {productInfo && (
                  <div style={{
                    ...
                    {!productInfo.canConfirm && productInfo.status < 1 && (
                      <p style={{ color: '#f59e0b', fontSize: 14 }}>
                        ⚠️ Waiting for Logistics to mark as Delivered
                      </p>
                    )}
                ...
                {lcInfo && (
                  <div style={{
                    ...
                    {!lcInfo.canConfirm && lcInfo.status < 3 && (
                      <p style={{ color: '#f59e0b', fontSize: 14 }}>
                        ⚠️ Waiting for delivery (Status must be DELIVERED_PENDING)
                      </p>
                    )}
                    {lcInfo.status >= 4 && (
                      <p style={{ color: '#10b981', fontSize: 14 }}>
